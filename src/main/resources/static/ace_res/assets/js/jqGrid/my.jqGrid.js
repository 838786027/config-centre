		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				var columns = $.extend([],$t.p.colModel),
                    ...
                    bD  ="<a id='"+fid+"_delete' class='fm-button ui-state-default ui-corner-all fm-button-icon-right ui-reset'><span class='fa fa-trash-o'></span>"+ p.Delete +"</a>",
				...
				bt = "<table class='EditTable' style='border:0px none;margin-top:5px' id='"+fid+"_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content' style='margin:1px'/></td></tr><tr><td class='EditButton' style='text-align:"+align+"'>"+bC+tmpl+"</td><td class='EditButton' "+butleft+">" + bD + bQ + bS + "</td></tr></tbody></table>";
				fid = $.jgrid.jqID( fid);
				$("#"+fid).jqFilter({
					columns : columns,
                ...
                $("#"+fid+"_delete").bind('click', function(){
                    var fl = $("#"+fid), sdata={}, res, filters;
                    fl.find(".input-elm:focus").change();
                    filters = fl.jqFilter('filterData');
                    if(p.errorcheck) {
                        ...
                        if (confirm("确认删除吗")) {
                           ...
                           var dt = "json";
                            ...
                            $.ajax($.extend({
                                ...
                                type : $t.p.mtype,
                                dataType : dt ,
                                data: $.isFunction($t.p.serializeGridData)? $t.p.serializeGridData.call($t,$t.p.postData) : $t.p.postData,
                                success:function(data,st, xhr) {
                                    alert("删除数据成功!");
                                    $($t).trigger("reloadGrid",[{page:1}]);
                                },
                            ...
                            },$.jgrid.ajaxOptions, $t.p.ajaxGridOptions));
                            delete $t.p.postData.oper;
                            $($t).trigger("reloadGrid",[{page:1}]);
                        } else {
                            return false;
                        }
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
				...
				$("<div class='"+ p.overlayClass+" jqgrid-overlay confirm' style='z-index:"+zI+";display:none;'>&#160;"+"</div><div class='confirm ui-widget-content ui-jqconfirm' style='z-index:"+(zI+1)+"'>"+p.saveData+"<br/><br/>"+bS + bS1 +bN+bC+"</div>").insertAfter("#"+frmgr);
